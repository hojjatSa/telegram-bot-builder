/**
 * @fileoverview Данные по умолчанию для нод — серверная копия для MCP/bot-tools
 * @module lib/bot-tools/node-presets
 */

import type { Node } from '@shared/schema';

/** Базовые поля dynamicButtons без зависимости от client */
const DEFAULT_DYNAMIC_BUTTONS = {
  sourceVariable: '',
  arrayPath: '',
  textTemplate: '{name}',
...
/**
 * Возвращает data по умолчанию для типа ноды
 * @param type - Тип ноды
 * @returns Объект data для инициализации
 */
export function getNodePresetData(type: Node['type']): Record<string, unknown> {
  const defaults: Partial<Record<Node['type'], any>> = {
    ...
    keyboard: {
      ...
      dynamicButtons: { ...DEFAULT_DYNAMIC_BUTTONS },
    },
    input: {
      inputType: 'any',
      inputVariable: '',
    ...
    set_variable: {
      assignments: [{ id: 'assign_1', variable: '', value: '', mode: 'text' }],
      autoTransitionTo: '',
